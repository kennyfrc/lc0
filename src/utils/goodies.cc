...
std::mutex goodies::qlock_;
std::queue<std::string> goodies::queue_;
...
void goodies::FillQ() {
    ...
    infile.close();
    qlock_.unlock();
    fprintf(stderr, "Read %lu epd's.\n", queue_.size());
}

std::string goodies::PopFen() {
    std::string retval;
    qlock_.lock();
    if (queue_.empty()) {
        retval = std::string();
