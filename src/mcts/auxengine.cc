...
void SearchWorker::AuxMaybeEnqueueNode(Node* n) {
  if (params_.GetAuxEngineFile() != "" &&
      ...
      n->GetAuxEngineMove() == 0xffff && !n->IsTerminal() && n->HasChildren()) {
...
void Search::AuxEngineWorker() {
      ...
      if (token == "uciok") {
        ...
        if (token == "name") {
          ...
          if (token == "SyzygyPath" && syzygy_tb_) {
            std::ostringstream oss;
            oss << "setoption name SyzygyPath value "
                << syzygy_tb_->get_paths();
            LOGFILE << oss.str();
            auxengine_os_ << oss.str() << std::endl;
          }
        }
  ...
  current_uci_ =
      "position fen " + current_position_fen_ + " moves " + current_uci_;
  ...
  while (!stop_.load(std::memory_order_acquire)) {
    {
      ...
      auxengine_cv_.wait(lock, [&] {
        return stop_.load(std::memory_order_acquire) ||
               !auxengine_queue_.empty();
      });
...
void Search::DoAuxEngine(Node* n) {
  ...
  if (pv_moves.size() == 0) {
    ...
    LOGFILE << "error: pv doesn't match bestmove:" << pv_moves[0] << " "
            << "bm" << bestmove_packed_int;
    pv_moves.clear();
    pv_moves.push_back(bestmove_packed_int);
  }
...
void Search::AuxUpdateP(Node* n, std::vector<uint16_t> pv_moves, int ply) {
  if (n->GetAuxEngineMove() < 0xfffe) {
    ...
    //  LOGFILE << "already done: curr:" << n->GetAuxEngineMove() << " new:" <<
    //  pv_moves[ply] << " (error? mismatch)";
    //} else {
  ...
  for (const auto& edge : n->Edges()) {
    if (edge.GetMove().as_packed_int() == pv_moves[ply]) {
      ...
      if (ply + 1 < params_.GetAuxEngineFollowPvDepth() &&
          ply + 1 < pv_moves.size() && edge.HasNode() && !edge.IsTerminal() &&
...
void Search::AuxWait() REQUIRES(threads_mutex_) {
  ...
  LOGFILE << "done waiting. auxengine_queue_ size " << auxengine_queue_.size()
          << " Average duration "
          << (auxengine_num_evals ? (auxengine_total_dur / auxengine_num_evals)
                                  : -1.0f)
          << "ms"
          << " Number of evals " << auxengine_num_evals << " Number of updates "
          << auxengine_num_updates;
